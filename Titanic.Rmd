...
train_cabin<-Rawtrain[,c(10,13,14,24,25:79)] 
#train_cabin<-Rawtrain[,c(10,13,14,23,24,79)]
train_cabin<-subset(train_cabin,is.na(Cabin_Charcter)==FALSE)
train_cabin$Cabin_Charcter<-as.factor(train_cabin$Cabin_Charcter)

test_cabin<-Rawtest[,c(9,12,13,22,23,24:78)] 
#test_cabin<-Rawtest[,c(9,12,13,22,23,78)]
...
modfit.rf <- randomForest(Cabin_Charcter ~ Fare+Pclass1+Pclass3+TicketNumbers , data=(subset(train_cabin,(is.na(train_cabin$TicketNumbers)==FALSE))))

predictions1 <- predict(modfit.rf, (subset(train_cabin,(is.na(train_cabin$TicketNumbers)==FALSE))), type = "class")
