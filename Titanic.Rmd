...
### Pclass Test
```{r}
kable((table(Rawtest$Pclass)) ,col.names = c("Pclass","Frequency"),escape = FALSE)%>%
  kable_styling(c("striped", "bordered"))

kable(table(Rawtest$Pclass1),col.names = c("Pclass1 - 1 - Yes 0 - No ","Frequency")) %>%
   kable_styling(c("striped", "bordered"))


kable(table(Rawtest$Pclass3),col.names = c("Pclass3 - 1 - Yes 0 - No ","Frequency")) %>%
   kable_styling(c("striped", "bordered")) 
...
### Completing Missing titles 
We can complete the missing titles based on Age and Sex 
...
We will try to build a machine learning algorithm to predict 
The cabin based on : 
1.Ticket
2.Fare
3.Pclass
We will train the model on the train set (using existing cabin data)
test it on the test set (using existing data), and then predict the 
missing data in train and test set
...
# Fare, Ticket numbers, P-class Dummy Variables, Ticket characters dummy variables 
# and Cabin character



#train_cabin<-Rawtrain[,c(10,13,14,23,24,25:79)] 
train_cabin<-Rawtrain[,c(10,13,14,23,24,25)]
train_cabin<-subset(train_cabin,is.na(Cabin_Charcter)==FALSE)
train_cabin$Cabin_Charcter<-as.factor(train_cabin$Cabin_Charcter)


#test_cabin<-Rawtest[,c(9,12,13,22,22,23,24:78)] 
test_cabin<-Rawtest[,c(9,12,13,22,23,24)]
test_cabin<-subset(test_cabin,is.na(Cabin_Charcter)==FALSE)
test_cabin$Cabin_Charcter<-as.factor(test_cabin$Cabin_Charcter)


#Create a Match matrix 
TrainTicketchr<-sort(unique(train_cabin$TicketChar))
TestTicketchr<-sort(unique(test_cabin$TicketChar))
...
for(i in 1:length(Missing_Items))
{
  count<-count + dim(test_cabin[test_cabin$TicketChar==Missing_Items[i],])[1]
  #Change the Missing Test items to U
  test_cabin[test_cabin$TicketChar==Missing_Items[i],]$TicketChar<-"U"
  
}
cat("Missing items",count)

df<-rbind(train_cabin,test_cabin)
df$TicketChar<-as.factor(df$TicketChar)

train_cabin$TicketChar<-df[1:nrow(train_cabin),]$TicketChar
test.index<-(nrow(train_cabin)+1):(nrow(train_cabin)+nrow(test_cabin))
test_cabin$TicketChar<-df[test.index,]$TicketChar
...
tree.model1 <- rpart(as.factor(Cabin_Charcter) ~ ., data=train_cabin, method="class", xval = 5,na.action = na.rpart)
plot(tree.model1,uniform=TRUE)
text(tree.model1,cex=0.75)

predictions1 <- predict(tree.model1 , train_cabin, type = "class")
confusionMatrix(predictions1, train_cabin$Cabin_Charcter)

predictions2 <- predict(tree.model1 , newdata = test_cabin, type = "class",na.action = na.rpart)
confusionMatrix(predictions2, test_cabin$Cabin_Charcter)
...
modfit.rf <- randomForest(Cabin_Charcter ~ Fare+Pclass1+Pclass3+TicketNumbers+(TicketChar) , data=train_cabin)

# Predict the testing set with the trained model
predictions1 <- predict(modfit.rf, train_cabin, type = "class")
...
# Predict the testing set with the trained model
predictions3 <- predict(modfit.rf, test_cabin, type = "class")

# Accuracy and other metrics
confusionMatrix(predictions3, test_cabin$Cabin_Charcter)
