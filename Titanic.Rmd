...
## Ticket
Ticket observations are comprised of a set of characters (or nothing)
and then numbers 
First building two new columns, one will have the characters and the other the numbers

```{r}
#Create a new colomn with Tickets which don't start with Numbers 
Rawtrain<-Rawtrain%>%
  mutate(TicketChar = ifelse(grepl("^[0-9]",Ticket),"N",Ticket))
#Remove all the digits 
#(Since the characters may have digits also we remove when there is occuarance of 3 or more )
Rawtrain$TicketChar<-gsub("[0-9]{3,}"," ",Rawtrain$TicketChar)
```


```{r}
p<-ggplot(subset(Rawtrain,TicketChar!="N"),aes(x=as.factor(TicketChar),group=as.factor(Survived),fill=as.factor(Survived)))+
  geom_bar()
